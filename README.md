...
| Visualizer flat but the rig responds, or the reverse | The scene is driving two universes. `fixture()` / `rgbStrip()` default to universe 0; `ch()`, `dim()`, `rgb()` write universe **1**. The strip draws the lowest one and says `(+1 more)` when there are others | Give the scene one universe: `uni(0, ch, v)` instead of `ch(ch, v)`, or a universe argument on the fixture. On the connector every universe is sent, but a **USB** interface carries only one — the run says which universes are not reaching it |
